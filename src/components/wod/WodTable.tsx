...
import { wodMock as initialWodMock } from "@/mocks/wod";

type WodRow = (typeof initialWodMock)[number];

export default function WodTable() {
  const [rows, setRows] = useState<WodRow[]>(initialWodMock);
  ...
  const [rowToDelete, setRowToDelete] = useState<WodRow | null>(null);
  ...
  const statusBadge = (s: WodRow["status"]) => {
    return (
      <Badge size="sm" color={s === "published" ? "success" : "warning"}>
      ...
      >
        <div className="text-center">
          ...
          <p className="text-sm leading-6 text-gray-500 dark:text-gray-400">
            {rowToDelete
              ? `Are you sure you want to delete "${rowToDelete.title}" (${rowToDelete.track} — ${rowToDelete.date})?`
