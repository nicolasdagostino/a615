...
import { PencilIcon, TrashBinIcon, AngleDownIcon, AngleUpIcon } from "@/icons";
...
export default function ProgramsTable() {
  ...
  const [rowToDelete, setRowToDelete] = useState<ProgramRow | null>(null);
  ...
  const confirmDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/programs?id=${encodeURIComponent(deletingId)}`, { method: "DELETE" });
      const json = await res.json().catch(() => ({}));
              ...
              <TableRow>
                <TableCell isHeader className="px-4 py-3 border border-gray-100 dark:border-white/[0.05]">
                  <div className="flex items-center justify-between cursor-pointer">
                    <p className="font-medium text-gray-700 text-theme-xs dark:text-gray-400">Program</p>
