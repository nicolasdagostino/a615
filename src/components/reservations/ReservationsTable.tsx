...
import { reservationsMock as initialReservationsMock } from "@/mocks/reservations";

type ReservationStatus = "confirmed" | "waitlist" | "cancelled" | "no-show";
...
const selectBaseClass =
  "w-full py-2 pl-3 pr-8 text-sm text-gray-800 bg-transparent border border-gray-300 rounded-lg appearance-none dark:bg-dark-900 h-9 bg-none shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800";

const inputBaseClass =
  "dark:bg-dark-900 h-9 w-full rounded-lg border border-gray-300 bg-transparent py-2 pl-3 pr-3 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800";
...
export default function ReservationsTable() {
  ...
  const [rows, setRows] = useState<ReservationRow[]>(
    initialReservationsMock as ReservationRow[]
  );

  // ✅ Days from classes
  const uniqueDays = useMemo(() => {
    const days = Array.from(new Set(classesMock.map((c) => c.day)));
    return days.length ? days : ["Mon"];
  }, []);

  const timesForDay = useMemo(() => {
    return (d: string) =>
      Array.from(
        new Set(classesMock.filter((c) => c.day === d).map((c) => c.time))
      ).sort();
  }, []);
  ...
  const initialDay = useMemo(() => {
    ...
    return qDay && uniqueDays.includes(qDay as any) ? qDay : uniqueDays[0];
  }, [searchParams, uniqueDays]);
  ...
  }, [searchParams, timesForDay, initialDay]);
  ...
  const availableTimes = useMemo(() => timesForDay(day), [day, timesForDay]);
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      const matchSlot =
        r.classItem.day === day && r.classItem.time === time && r.date === date;
  ...
  const openDelete = (r: ReservationRow) => {
    setRowToDelete(r);
  ...
  const badgeColor = (s: ReservationStatus) => {
    if (s === "confirmed") return "success";
    if (s === "waitlist") return "warning";
    return "error";
  };

  const labelStatus = (s: ReservationStatus) => {
  ...
  const editHref = (id: number) =>
    `/admin/reservations/${id}/edit?day=${encodeURIComponent(day)}&time=${encodeURIComponent(
      time
        ...
        <div className="flex flex-col gap-3 px-4 py-4 border border-b-0 border-gray-100 dark:border-white/[0.05] rounded-t-xl">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              <Link
                href={`/admin/reservations/new?day=${encodeURIComponent(day)}&time=${encodeURIComponent(
                  time
                )}&date=${encodeURIComponent(date)}`}
          ...
          <div className="grid grid-cols-1 gap-3 lg:grid-cols-12">
            <div className="lg:col-span-3">
              <label className="mb-1 block text-xs font-medium text-gray-500 dark:text-gray-400">Day</label>
              <div className="relative">
                <select className={selectBaseClass} value={day} onChange={(e) => onChangeDay(e.target.value)}>
                  {uniqueDays.map((d) => (
                    <option key={d} value={d} className="text-gray-500 dark:bg-gray-900 dark:text-gray-400">
                      {d}
                    </option>
                  ))}
                </select>
            ...
            <div className="lg:col-span-3">
              ...
              <div className="relative">
                <select className={selectBaseClass} value={time} onChange={(e) => onChangeTime(e.target.value)}>
                  {availableTimes.map((t) => (
                    <option key={t} value={t} className="text-gray-500 dark:bg-gray-900 dark:text-gray-400">
                      {t}
                    </option>
                  ))}
                </select>
            ...
            <div className="lg:col-span-3">
              ...
              <input type="date" className={inputBaseClass} value={date} onChange={(e) => onChangeDate(e.target.value)} />
            </div>
          ...
          <div className="text-sm text-gray-500 dark:text-gray-400">
            Roster for <span className="font-medium text-gray-800 dark:text-white/90">{day}</span>{" "}
            <span className="font-medium text-gray-800 dark:text-white/90">{time}</span>{" "}
            — <span className="font-medium text-gray-800 dark:text-white/90">{date}</span>
          </div>
        </div>
            ...
            <Table>
              ...
              <TableBody>
                ...
                {currentData.length === 0 && (
                  <TableRow>
                    <TableCell className="px-4 py-6 text-sm text-gray-500 dark:text-gray-400" colSpan={3}>
                      No reservations found for this roster.
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
      ...
      >
        <div className="text-center">
          ...
          <p className="text-sm leading-6 text-gray-500 dark:text-gray-400">
            {rowToDelete
              ? `Are you sure you want to delete the reservation for ${rowToDelete.member.name}?`
